        ...
        <div className="orders-list">
          {orders.map((order) => (
            <Link href={`/cuenta/pedidos/${order.id}`} key={order.id}>
              <span><b>{order.order_number}</b><small>Pago: {paymentStateLabel(order.payment_state)}</small></span>
              <strong>{orderStateLabel(order.order_state)}</strong>
              <time dateTime={order.created_at}>{formatStoreDate(order.created_at)}</time>
              <ArrowRight size={17} />
            </Link>
          ))}
        </div>
